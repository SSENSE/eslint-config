...
Also, `this` actively thwarts function composition and functions-as-values (e.g. arguments to higher order functions) because when executed they would lose their `this` context. The canonical solution would be [`.bind`](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_objects/Function/bind) but that burdens the programmer and degrades readability.
...
```
**Pass**
Avoid `this` so that function composition and functions-as-values is possible.
```js
const object = {
  numbers: [1, 2, 3],
  sum: () => (
